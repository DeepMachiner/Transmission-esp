            ...
            { 
                ...
                {                           // Test each time just in case the MPU is writing to the FIFO Buffer
                    ...
                    {                                                 // fifo count will reach zero so this is safe
                        RemoveBytes = min<int>((int)fifoC, BUFFER_LENGTH); // Buffer Length is different than the packet length this will efficiently clear the buffer
...
void MPU6050::PrintActiveOffsets(int16_t *offsets)
{
    ...
    if (offsets != NULL)
    {
        memcpy(offsets, Data, sizeof(Data));
    }
    ...
    if (offsets != NULL)
    {
        memcpy(&offsets[3], Data, sizeof(Data));
    }
}
